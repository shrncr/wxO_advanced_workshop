...
Line 1 – This lets you use the `@tool` decorator to turn your function into a usable tool.

Line 2 – This gives you access to the Tavily credentials you set up in the previous lab.

Line 3 – This imports the Tavily Python module so you can interact with the Tavily API.
...
5\. Add a header to the top of your function. This will describe the tool, and explain the inputs and outputs of the function
...
13\. Since we used a non-native Python module in our code, we need to add its name to the `requirements.txt` file. This file lists all the external packages your code needs to run.\
...
```bash
orchestrate tools import -f ./tools-lab/tools/web_search_tool/web_search.py -r ./tools-l\\ ab/tools/web_search_tool/requirements.txt -k python --app-id tavily
```
